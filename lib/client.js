  ...
  factory: (require) => {
    ...
    function LensTab({ ctx: _ctx, scope }) {
      ...
      return React.createElement('div', { style: { padding: 12 } },
        ...
        stats ? React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)' } }, `${en.saved} ${stats.savedTokens} ${en.tokens} (${stats.savedPercent}%) · ${stats.calls} ${en.ops}`) : React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)' } }, en.noData),
        stats && stats.budgetLimit ? React.createElement('div', { style: { marginTop: 8 } },
          React.createElement('div', { style: { fontSize: 11, color: stats.lowBudget ? '#d73a4a' : 'var(--dsw-alias-label-secondary)', marginBottom: 4 } }, `${en.budget} ${stats.budgetUsed}/${stats.budgetLimit} ${en.tokens} (${stats.budgetPercent}%)${stats.lowBudget ? ' ⚠' : ''}`),
        ...
        history.length ? React.createElement('div', { style: { marginTop: 10 } },
          React.createElement('div', { style: { fontSize: 11, color: 'var(--dsw-alias-label-tertiary)', marginBottom: 4 } }, en.history),
        ...
        React.createElement('div', { style: { marginTop: 12, display: 'flex', gap: 8 } },
          React.createElement('button', { onClick: refresh, style: { fontSize: 12, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--dsw-alias-border-l2)', background: 'var(--dsw-alias-bg-layer-3)', color: 'var(--dsw-alias-label-primary)', cursor: 'pointer' } }, en.refresh)
        )
      );
    }
      ...
      const popover = open ? React.createElement('div', {
        ...
        hasSavings
          ? React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)', marginBottom: 8 } },
              `${en.saved} ${stats.savedTokens} ${en.tokens} (${stats.savedPercent}%) · ${stats.calls} ${en.ops}`
            )
        ...
        stats && stats.budgetLimit ? React.createElement('div', { style: { marginTop: 6, marginBottom: 8 } },
          React.createElement('div', {
            ...
            React.createElement('span', null, en.budget),
