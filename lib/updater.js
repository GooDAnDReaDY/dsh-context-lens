...
import { writeJson, isTrustedUpdateRequest } from './http.js';
...
function profileNameFromArgv(argv) {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (typeof argv[index] === 'string' && argv[index].startsWith('--profile=')) {
...
async function installExact(target, packageSpec, options) {
  ...
  await new Promise((resolvePromise, reject) => {
    // Note: per issue #61 / dsh-clinebot#24 / dsh-agent-loop-guard#38, do NOT disable pnpm minimum release age quarantine
    ...
    handler: async (request, response) => {
      try {
        ...
        if (!isTrustedUpdateRequest(request)) {
          writeJson(response, 403, { ok: false, error: 'Rejected non-local or cross-origin update request.' });
          return;
        }
        if (installing) {
        ...
        writeJson(response, 500, { ok: false, error: String(error?.message || error) });
      }
    },
  });
}