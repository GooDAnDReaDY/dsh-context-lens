...
test('#64: client.js declares configForms in inject and has 0 references to lanSettings', () => {
  assert.ok(clientSrc.includes("module.exports.inject = ['slots', 'locale', 'configForms']"));
  assert.ok(!clientSrc.includes('lanSettings'), 'Must not reference foreign lanSettings');
  assert.ok(clientSrc.includes('_ctx.configForms'));
});
